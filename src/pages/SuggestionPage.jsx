...
// ClubLabDetail không cần import ở đây nữa, vì nó sẽ được hiển thị ở trang riêng

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

// formFields và initialFormData giữ nguyên như trong SuggestionFeature.jsx cũ
const formFields = [
  {
    name: "fullName",
    label: "Họ và tên",
...
export default function SuggestionPage() {
  ...
  const handleSubmitSuggestions = async (e) => {
    ...
    setSuggestions([]);
    // setSelectedClubForDetail(null); // Không cần nữa
  ...
  const handleSuggestionClick = (clubId) => {
    navigate(`/clubs/${clubId}`); // Điều hướng đến trang chi tiết
  };
  ...
  // selectedClubForDetail và logic liên quan không còn ở đây

  return (
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      <Link // Sử dụng Link để quay lại trang chủ
        to="/"
          ...
          {suggestions.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {suggestions.map((suggestion) => (
                <div
                  key={suggestion._id}
                  // Bọc bằng Link hoặc dùng onClick + navigate
                  className="p-4 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-xl transition-shadow group"
                >
                  <Link to={`/clubs/${suggestion._id}`} className="block">
                    {" "}
                    {/* Bọc bằng Link */}
